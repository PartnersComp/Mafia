...
import { StyleSheet, Text, View, TextInput, Button,Dimensions,TouchableOpacity,Image } from 'react-native';
...
const Login = ({navigation}) =>{
  ...
  return(
    <View style={styles.container}>
      ...
      <Text style={styles.textStyle}>Enter you nickname to enter the world of </Text>
      <Text style={styles.textStyle}>MAFIAS and VILLAGERS </Text>
      <View style={{height : 50}}></View>
      <TextInput style={{color : 'white', height: 60, width :Dimensions.get('window').width - 150,borderColor: 'white', borderBottomWidth: 1 }}value={name} textAlign="center" onChangeText={(text)=>{setName(text)}}></TextInput>
...
const Room = ({route,navigation}) =>{
  ...
  return(
    <View style={styles.container}>
      <Text style={styles.textStyle}>Create you own village</Text>
      <View style={{height : 20}}></View>
      <Text style={styles.textStyle}>OR</Text>
      <View style={{height : 20}}></View>
      <Text style={styles.textStyle}>Join an exisiting village</Text>
      <View style={{height : 20}}></View>
      ...
      <View style={{height : 20}}></View>
      <TouchableOpacity style={{flexDirection : 'row',alignItems : 'center'}}><Text style={styles.touchableButton} onPress={() => setOverLay({visible : true,type : 'join'})}> Join   </Text></TouchableOpacity>
      <Overlay isVisible={overLay.visible} onBackdropPress={() => setOverLay({visible : false,type : overLay.type})}>
          {
            ...
            <View style={{felx : 1,justifyContent : 'center',alignItems :'center'}}>
              <TextInput style={{color : 'black', height: 40, width :Dimensions.get('window').width - 150,borderColor: 'black', borderBottomWidth: 1 }}value={code} textAlign="center" placeholder="Enter the code" onChangeText={(text)=>{setCode(text)}}></TextInput>
              <TouchableOpacity style={{paddingTop : 30,paddingBottom : 10}} onPress={() =>joinRoom()}><Text>Join</Text></TouchableOpacity>
            </View>
            :
            <View  style={{felx : 1,justifyContent : 'center',alignItems :'center'}}>
              <TextInput style={{color : 'black', height: 40, width :Dimensions.get('window').width - 150,borderColor: 'black', borderBottomWidth: 1 }}value={roomName} textAlign="center" placeholder="Enter the Room Name" onChangeText={(text)=>{setRoomName(text)}}></TextInput>
...
export default function App() {
  ...
  React.useEffect(() =>{
    ...
    Firebase.auth().onAuthStateChanged((userToken) =>{
      var user=userToken ? {id : userToken.uid} : {id : ""};
